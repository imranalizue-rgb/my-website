...
const footerLinks = {
  ...
  Services: [
    ...
    { to: '/services#digital-strategy', label: 'Digital Strategy' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
            ...
            <div className="flex gap-3">
              {Object.entries(company.social).map(([platform, url]) => (
                <a
                  ...
                  className="w-10 h-10 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-red-500 hover:border-red-600/40 transition-colors"
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.to}>
                    <Link
                      to={link.to}
                      className="text-zinc-400 text-sm hover:text-red-500 transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-zinc-400">
              <li>
                <a href={`mailto:${company.email}`} className="hover:text-red-500 transition-colors">
                  {company.email}
                </a>
              </li>
              <li>
                <a href={`tel:${company.phone.replace(/\s/g, '')}`} className="hover:text-red-500 transition-colors">
                  {company.phone}
                </a>
              </li>
